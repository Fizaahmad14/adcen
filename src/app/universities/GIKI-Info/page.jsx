...
const BlogComponent = () => {
  ...
  return (
    <div>
      ...
      <img src="/images/universities/giki.jpg" alt="Blog Cover" className="blog-cover" />
          ...
          {Object.keys(jsonData).map((key) => {
            return (
              <div key={key}>
                ...
                {key === "Why GIKI" ? (
                  <p className="blog-content" style={{ whiteSpace: 'pre-line' }}>
                    {/* Render HTML with bold headings using dangerouslySetInnerHTML */}
                    <span dangerouslySetInnerHTML={{ __html: formatContent(jsonData[key]) }} />
                  </p>
                ) : (
                  <div className="blog-content" dangerouslySetInnerHTML={{ __html: formatContent(jsonData[key]) }} />
                )}
                {/* Add a horizontal rule after each item except the last one */}
                {key !== Object.keys(jsonData)[Object.keys(jsonData).length - 1] && <hr />}
              </div>
            );
          })}
        </div>
